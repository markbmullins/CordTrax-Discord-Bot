...
async function createEmbed(string){
		...
		let queuesEmbed = new Discord.RichEmbed()
		.addField("Queues:", `${string}`)
		.setColor("#15f153");
		return queuesEmbed;
}
...
module.exports.run = async (client,message,args,prefix,con_database) => {
	...
	if(args[0] === "help"){
		...
		return message.reply(`Invalid use of the myqueues command. Type ${prefix}myqueues help for help.`);
	}
	else{
		var queues = await queueFunctions.getAllQueues(message, con_database);
		if(queues.length === 0){return message.reply("You don't have any queues. Please use the add command to create some.")}
