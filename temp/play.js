...
function playQueue(client, connection, message) {
  ...
  client.queue.dispatcher = connection.playStream(
    YTDL(client.queue[client.queueIndex], { filter: "audioonly" }),
    streamOptions
  );
  client.queueIndex += 1;
...
module.exports.run = async (client, message, args, prefix, con_database) => {
  ...
  if (args[1]) {
    ...
    //[✔] if !play <URL>
      ...
      if (!client.queue[0]) {
        return message.reply(
          `That queue does not exist. Use the add command to create a new queue or ${prefix}play help for help.`
        );
      }
