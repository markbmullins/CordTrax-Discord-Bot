...
module.exports.run = async (client, message, args, prefix, con_database) => {
  ...
  if (args[0] === "help") {
    ...
    return message.reply(
      `Please input a queue name to delete. Use ${prefix}delete help for help using the deletequeue command.`
    );
  } else if (args[1]) {
    return message.reply(
      `Invalid use of the deletequeue command. Use ${prefix}deletequeue help for help.`
    );
  } else {
      ...
      try {
        ...
        try {
          var response = await message.channel.awaitMessages(
            message2 =>
              message2.content.toLowerCase() === "yes" || message2.content.toLowerCase() === "no",
            {
              maxMatches: 1,
              time: 5000,
              errors: ["time"],
