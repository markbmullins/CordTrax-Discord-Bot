...
client.on('message', async message => {
  ...
  if (cmd.substring(0, prefix.length) != prefix) {
    ...
    con_database.query(
      `SELECT * FROM xp WHERE id = '${message.author.id}' and guildid = '${
        message.guild.id
      }'`,
      (err, result) => {
        if (err) throw err;
        let query;
        if (!result.rows[0]) {
          query = `INSERT INTO xp (id, xp, guildid) VALUES ('${
            message.author.id
          }', ${generateXp()},'${message.guild.id}')`;
        } else {
          let xp = result.rows[0].xp;
          query = `UPDATE xp SET xp = ${xp + generateXp()} WHERE id = '${
            message.author.id
  ...
  //else if message does start with prefix
    ...
    if (commandfile)
      commandfile.run(client, message, args, prefix, con_database);
    else {
      return message.reply(`Invalid command. Type ${prefix}help for help.`);
    }
  } //end else if
  else {
    return;
